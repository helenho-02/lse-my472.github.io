...
```{r}
library(tidyverse)
my.file <- "/Users/helenho/Local/lse-my472.github.io/week04/data/news-article-1.txt"
raw <- read_file(my.file, locale = locale(encoding="Shift-JIS"))
print(raw) # this is Japanese[language]
my.utf.file <- "/Users/helenho/Local/lse-my472.github.io/week04/data/news-article-1-utf8.txt"
write_file(raw, my.utf.file)
```
...
str_view(text, "\\b[A-Z]{1,2}[0-9][A-Z0-9]?[ ]?[0-9][A-Z]{2}\\b") 
```
...
for(document in data_char_ukimmig2010){
  print(substr(document, 1, 80)) ## There are a bunch of ways to do this!
}
## 3. That did not tell us which party produced which text. Now print the party names before the first 80 characters:
for(party in names(data_char_ukimmig2010)){
  print(paste0(party,": ",substr(data_char_ukimmig2010[party], 1, 80))) ## There are a bunch of ways to do this!
}
```
...
my.dfm <- data_char_ukimmig2010 %>%                     # 1. start with the data
  ...
  dfm() %>%
  dfm_trim(min_docfreq = 2)                                 # 5. minimum word frequency = 2
my.dfm
```
...
```{r}
my.dfm %>% 
  dfm_select(pattern = "work", valuetype = "regex") %>% 
  featnames()
```
...
```{r}
simple_dict <- list(economic = c("work", "employ"), 
                    legal = c("asylum", "illeg", "deport"))
simple_dict <- dictionary(simple_dict)
simple_dict
```
...
```{r}
dfm_lookup(my.dfm, dictionary = simple_dict) 
dfm_lookup(my.dfm, dictionary = simple_dict)/rowSums(my.dfm)
```
